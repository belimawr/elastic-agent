...
func TestOTelElasticsearchInvalidAPIKeyBackoff(t *testing.T) {
	...
	expectedAuthorization := "ApiKey " + base64.StdEncoding.EncodeToString([]byte(invalidAPIKey))
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost && r.URL.Path == "/_bulk" {
			mu.Lock()
			bulkRequestTimes = append(bulkRequestTimes, time.Now())
			invalidKeyWasSent = invalidKeyWasSent || r.Header.Get("Authorization") == expectedAuthorization
			mu.Unlock()
